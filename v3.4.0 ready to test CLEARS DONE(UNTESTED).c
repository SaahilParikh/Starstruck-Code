...
void driveForwardPid(int Encoder, int minSpeed = 20)
{
	...
	int speed;
	int bias = 3;
	int threshold = 40; // TUNE THIS VALUE - 2/13/2017
	...
	while(nMotorEncoder[driveRightBack] < Encoder && nMotorEncoder[driveLeftBack] < Encoder)
	{
		...
		if(abs(nMotorEncoder[driveRightBack]) < abs(nMotorEncoder[driveLeftBack])-bias) // If the left side has more encoder ticks than the right
		...
		else if(abs(nMotorEncoder[driveRightBack])-bias > abs(nMotorEncoder[driveLeftBack]))	// Other  way around from top comment
		{
...
void driveBackwardPid(int Encoder, int minSpeed = -90)
{
	...
	while(nMotorEncoder[driveRightBack] > -Encoder && nMotorEncoder[driveLeftBack] > -Encoder)
	{
		...
		if(abs(nMotorEncoder[driveRightBack])-bias < abs(nMotorEncoder[driveLeftBack])) // If the left side has more encoder ticks than the right
		{																																			// Make the right side faster, Has bias
			motor[driveRightFront] = speed;
			motor[driveRightBack] = speed;
			motor[driveLeftFront] = speed/bias;
		...
		else if(abs(nMotorEncoder[driveRightBack]) >abs( nMotorEncoder[driveLeftBack])-bias)	// Other  way around from top comment
		{
			motor[driveRightFront] = speed/bias;
			motor[driveRightBack] = speed/bias;
			motor[driveLeftFront] = speed;
...
void gyroTurn(int degrees, char direction, int minSpeed = 30)
{
	...
	while(!targetReached)
	{
		error = ((dir)*degrees - abs(SensorValue(gyro)));  // Using YAW Rate Gyroscope to find the error: units/ticks away from the target
		if(abs(error) > 100 || abs(error)<30)
		{
			integral = 0;
		}
		else
	...
	while(nMotorEncoder[liftLeftEnc] < releaseThreshold+150 || nMotorEncoder[liftRightEnc] < releaseThreshold+150)
		{
			...
			openClaw();
			motor[liftLeftEnc] = 20;
			motor[liftLeftIn] = 20;
			motor[liftLeftTop] = 20;
			motor[liftRightEnc] = 20;
			motor[liftRightIn] = 20;
			motor[liftRightTop] = 20;
...
task autonomous()
{
  ...
  //
    ...
    lift2Bottom();
		...
		clearDrive();
		driveForwardPid(850);                            // Forward to line up with middle cube
		haultDrive(UH);
		clearDrive();
		clearGyro();
		...
		driveForwardPid(2000, 110);                        // Drive into Cube
		haultDrive(UH);
		clearDrive();
		pickUp();                                        // Pick cube up
		startTask(liftHolder);
    ...
    lift2Bottom();
		...
		clearDrive();
		driveForwardPid(550);                            // Go for Second Ground Cube
		haultDrive(UH);
		pickUp();
		startTask(liftHolder);
		clearDrive();
		...
		clearDrive();
		driveForwardPid(50, 20);                            // go forward to not hit fence
		haultDrive(100);
		clearGyro();
		gyroTurn(900, 'r')                                 // Turn Right to line up with stars
		haultDrive(100);

		clearDrive();
		driveForwardPid(300, 60);                           // Drive into the front three stars
		haultDrive(10);
		...
		clearGyro();
		gyroTurn(900, 'l')                                 // Turn Left to line up with fence
		haultDrive(100);
